...
#[test]
fn mixed_package_kmdf_workspace() {
    with_file_lock(|| {
...
fn build_driver_project(driver_type: &str) {
    ...
    assert!(stdout.contains(&format!(
        "Processing completed for package: {driver_name}"
    )));
...
fn verify_driver_package_files(driver_or_workspace_path: &str, driver_name: &str, driver_binary_extension: &str) {
    ...
    fn assert_dir_exists(path: &str) {
        ...
        assert!(path.is_dir(), "Expected {} to be a directory", path.display());
    }
